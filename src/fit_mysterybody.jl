...
# @show inds
  ...
  for i=1:nmu
   ...
   # ax4.plot(wp3["per"]./365.35,actual_logL(wp3["lprob_per"][:,i],data),label=string(round(mu[i],sigdigits=2)))
    push!(handles,h)
  end
  # ax3.set_xlim(10,13)
  ...
  #fig.legend(title=string(L"$\gamma$ values"),fontsize="medium",title_fontsize="medium",loc="lower right",ncol=2,bbox_to_anchor=(0.102,0.025,0.885,.102))
   # fig.legend(loc="upper right",title=string(L"$\gamma$ values"),fontsize="medium",title_fontsize="large",bbox_to_anchor=(0.05,0.1,0.885,0.05))
  ax1.legend(loc="upper right",title=string(L"$\gamma$ values"),fontsize="medium",title_fontsize="large",ncol=2,bbox_to_anchor=(0.05,0.9,0.95,0.05))
  fac(true_per)=true_per + true_per/100
  ax1.text(fac(1.89),-162,"Mars",color="black")
  ax1.text(fac(11.86),-162,"Jupiter",color="black")

  ax1.set_title(string(L"$σ_\mathrm{obs}$","=",sigma," s",'\t',L"$n_\mathrm{year}$","=",nyear),loc="right")
  # ax1.text(12.5,-165,string(L),fontsize="large")
   # ax4.text(0.02,-0.05,string(L"$σ_{obs}$","=",sigma," s",'\n',L"$n_{year}$","=",nyear,'\n',L"$m_p/ M_{\odot}$=",L"$10^γ$"),color="black",fontsize="medium")
  ax1.text(15.3,-163,string(L"$m_p/ M_{\odot}$=",L"$10^γ$"),color="black",fontsize="large")
  ax1.set_ylim(-220,-160)
  ax1.minorticks_on()
 
 # ax3.set_ylabel("Relative Prob.",fontsize="medium")
  # ax2.set_ylabel("Relative Probability",fontsize="large")
  # ax3.set_title("With respect to all mass-ratios")
  ax1.set_ylabel(string("ln",L"$\mathcal{L}$"),fontsize="large")
  # ax3.tick_params(axis="both",top=true,direction="in")
  # ax2.tick_params(axis="both",top=true,direction="in")
  # ax2.minorticks_on()
  #ax3.minorticks_on()

  ax1.set_xlabel("Orbital Period [yrs]",fontsize="large")
  # ax2.set_xlabel("Orbital Period [yrs]",fontsize="large")
  fig.subplots_adjust(hspace=0.0,wspace=0.2,left=0.05,right=0.98,top=0.95)
 ...
 # fig.supxlabel("Orbital Period [years]",fontsize="x-large")
  ...
  tight_layout()
  savefig("2025/2025actual_logL_jup_norm_60_30_.png",dpi=150)
#end
#plot_mu()
#show()
