...
function nlog_prior(x::Array{T,1}) where {T <: Real}
...
for i=2:n
  ...
  nlprior += log(ecc)  # Add this to negative log prior
...
grad_nlog_prior= x -> ForwardDiff.gradient(nlog_prior,x)
hess_nlog_prior= x -> ForwardDiff.hessian(nlog_prior,x)
