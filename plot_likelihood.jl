...
function plot_profile(xgrid::Array{Float64,1},lprob::Array{Float64,1},truex::Float64,nbins::Int,run::Array{Float64,1},color::String,pname::String)
    ...
    xprob=exp.(lprob .- maximum(lprob))
		ax1.plot(xgrid ./365.25,xprob,color=color,label=string(run)) 
		ax1.axvline(truex/365.25,linestyle="--",color="black",label=string(truex))
		ax1.set_xlabel("Planet Period Search Grid [years]")
		ax1.text(truex/365.25 + .1,1.01,pname)
