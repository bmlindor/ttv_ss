...
# should equal the dimension of Y):
  ...
  # Set up array for computing coefficients:
    A = zeros(typeof(X[1,1]),sx[1],sx[1])
    B = zeros(typeof(X[1,1]),sx[1])
  # Compute A = \sum_i \sum_j \sum_k X_{i,k} X_{j,k}/\sigY_k^2; builds matrix A
    for i = 1:sx[1]
      for j = 1:sx[1]
        for k = 1:sx[2]
  ...
  # Compute B = \sum_i \sum_k X_{i,k} Y_k/\sigY_k^2; builds vector B
      for k = 1:sx[2]
