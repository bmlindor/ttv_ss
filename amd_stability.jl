...
Petit, Laskar & Boue (2018): https://arxiv.org/pdf/1806.08869.pdf
=#
...
function AMD_stability(μ1::Real,μ2::Real,a1::Real,a2::Real,Cx::Real)
    ...
    # Relative AMD from Hill stability condition (Petit, Laskar, & Boue 2018):
	C_Hill_30 = relative_AMD_Hill_stability(μ1, μ2, a1, a2)
	C_Hill_26 = relative_AMD_Hillstability(μ1, μ2, a1, a2)

    # Final result:
    C_crit = min(C_coll, C_mmr)
	ratio = Cx/C_crit
	if Cx < C_crit #&& 	Cx < min(C_Hill_26,C_Hill_30)
		return :Stable, ratio
	elseif C_coll < C_mmr
		return :Collision, ratio
	elseif Cx < min(C_Hill_26,C_Hill_30) && Cx < C_crit 
		return :Hill_Stable, ratio
	else
...
"""
    relative_AMD_Hill_stability(μ1, μ2, a1, a2)

Compute the critical relative AMD for Hill stability, based on Equation 30 in Laskar, Petit, & Boue (2018).
...
# Returns:
- `C_H::Float64`: critical relative AMD for Hill stablity.
"""
function relative_AMD_Hill_stability(μ1::Real, μ2::Real, a1::Real, a2::Real)
    @assert(a1 <= a2)
    ...
    C_Hill = γ*sqrt(α) + 1 - (1+γ)^(3/2)*sqrt((α/(γ+α))*(1+((3^4*ϵ^2*γ^3)^(1/3))/(1+γ)^2))
    return C_Hill
end
...
"""
function relative_AMD_Hillstability(μ1::Real, μ2::Real, a1::Real, a2::Real)
    @assert(a1 <= a2)
