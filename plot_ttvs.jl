...
function plot_res(sigma::Real,nyear::Real,options,include_moon::Bool=false)
  ...
  plot(ttsim1,ttv1-(p2_ttvs[1,2,1:n1]),label=L"$\mathcal{H}_{PP}$")
  ...
  plot(ttsim2,p2_ttvs[2,1,1:n2],label=L"$\mathcal{H}_{PP}$")
  # plot(ttsim2,p3_ttvs[2,3,1:n2]+p3_ttvs[2,1,1:n2],label=L"$\mathcal{H}_{PPP}$")
  ax3.text(-1,6.5,label,fontweight="bold")
  ylim(-9,9)
  ylabel("O - C [mins]")
  xlabel("Time [yrs]")
  minorticks_on()
  ...
  plot(ttsim2,ttv2-(p2_ttvs[2,1,1:n2]),label=L"$\mathcal{H}_{PP}$")
  ...
  # errorbar(ttsim1,ttv1,sigtt1,fmt=".",color="black",mec="black",mfc="white")#,label="Venus")
  # errorbar(ttsim2,ttv2,sigtt2,fmt=".",color="black",mec="black",mfc="white")#,label="Earth")
  ax1.plot(ttsim1,p3_ttvs[1,3,1:n1]+p3_ttvs[1,2,1:n1],label=L"$\mathcal{H}_{PPsP}$")
  ax3.plot(ttsim2,p3_ttvs[2,3,1:n2]+p3_ttvs[2,1,1:n2]+moon3,label=L"$\mathcal{H}_{PPsP}$")
  # ylabel("TTVs [mins]")
  # xlabel("Time [yrs]")
  # minorticks_on()
  # tick_params(which="both",direction="in",top=true,right=true)
  # legend(loc="upper right")
  # subplot(212)
  ax2.plot(ttsim1,ttv1-(p3_ttvs[1,3,1:n1]+p3_ttvs[1,2,1:n1]),label=L"$\mathcal{H}_{PPsP}$")
  ax4.plot(ttsim2,ttv2-(p3_ttvs[2,3,1:n2]+p3_ttvs[2,1,1:n2]+moon3),label=L"$\mathcal{H}_{PPsP}$")
...
function plot_contrib(sigma::Real,nyear::Real,options::Array{String},include_moon::Bool=false)
  ...
  sim_obs_label= string(L"$\sigma_{obs}=$",sigma," sec")
  ax1.set_title("Fit to 3-planets+Moon",fontsize="x-large")
  ax1.text(maximum(ttsim1)-5,5,sim_obs_label)
  # ax2.text(maximum(ttsim1)-5,5,sim_obs_label)
  # ax1.legend(title="Source of contributions to TTV",fontsize="medium",title_fontsize="medium",markerscale=1.5,bbox_to_anchor=(0.,1.02,1.,.102),loc="lower left",
  #     ncol=3,mode="expand",borderaxespad=0.0)
  # ax1.legend(bbox_to_anchor=(0.,1.02,1.,.102),loc="lower left",
  #     ncol=3,mode="expand",borderaxespad=0.0,fontsize="large")
  ax1.legend(loc="lower left",fontsize="large",title="Contributions",title_fontsize="large",ncol=4)
  ax2.legend(bbox_to_anchor=(0.,1.02,1.,.102),loc="lower left",     ncol=nplanet,mode="expand",borderaxespad=0.0,fontsize="large")
  # tight_layout()
  # legend(loc="upper right")
